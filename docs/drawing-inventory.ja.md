...
計242件の観測値は、画面に表示される要素数ではありません。NUL・サロゲートペア・書式文字も保持します。
...
exampleは `{ "inventory": ..., "preview": ... }` を出力します。
...
固定したSampleBg回帰入力では、4つの配置参照から**線分・折れ線92件、円・円弧11件、文字53件**を生成します。
画像ノード2件と未接続の文字2件は省略します。診断用のXY表示で、表題欄と4部品の輪郭を配置できる段階です。
部品ビュー周囲に余分な矩形が残り、可視性・補助図形・線種と線幅・画像とロゴ・文字整列・
フォント実寸・現在状態・正式なシート順序・単位は未確定です。
ブラウザーでの円弧分割や代替フォントは表示用の近似で、解析的なシーンには含めません。
保存サムネイルは見比べるための資料であり、現在状態のAutodesk比較値ではありません。
他のmajor31構造やmajor23/24、製品版IDWビューアーの対応を保証しません。
