...
| 28 | `mateolikescats.idw` | 1 / 5 | 1,816 | RGBA。保存キャッシュ1件を確認 |
| 26 | `RespiraWorks.idw`、`starliliko.idw` | 1 / 3、1 / 0 | 529、90 | RGBA |
| 26 | `RespiraWorks-bottom-assembly.idw`、`RespiraWorks-filter-panel-assembly.idw` | 1 / 3、1 / 4 | 528、9,475 | RGBA。保存キャッシュ2件を確認 |

点列の型番号は、そのセグメント内の型テーブルから解決します。major26・28・29で観測した
三角形列は、保存された矢印形状として塗りつぶして表示します。major28では観測済みの
スプライン・楕円・注記配置の形式も追加しました。filter-panel assemblyは部分SVGを出力できますが、
ViewerのシートJSONは32 MiBを超えるため、容量超過を診断して表示を拒否します。
