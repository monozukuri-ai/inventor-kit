...
対応するmajor31の白黒ビュー画像とmajor23・28のカラーRGBAビュー画像は保存画素からPNGへ変換し、そのSHA-256と元レコードの出典を保持します。
...
| --- | --- | --- | --- | --- |
| 24 | `Template_IACS.idw` | 1 / 0 | 50 | 未対応。モデルビューは未観測 |
| 29 | `Toys-R-Us-Rex.idw` | 1 / 3 | 156 | 未対応 |
| 28 | `mateolikescats.idw` | 1 / 5 | 1,411 | RGBA。保存キャッシュ1件を確認 |
| 26 | `RespiraWorks.idw`、`starliliko.idw` | 1 / 3、1 / 0 | 270、90 | 未対応 |

この表は固定した実ファイルの回帰検証結果であり、Autodeskの描画との照合結果ではありません。
全て`experimental_partial`です。未知の曲線形式・色・フォント・未解決参照は、省略理由や
フォントの代替表示として明示します。major26の一部の円・円弧suffixは未対応です。
...
既存のmajor23・31を含む7図面、158,245レコードを回帰inventoryで検証します。
