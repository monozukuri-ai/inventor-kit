...
type Font = { family: string; height_candidate: number; weight_candidate: number; width_factor: number | null };
type Geometry =
  | { kind: 'polyline'; points: Point[] }
  | { kind: 'curve'; center: Point; u: Point; v: Point; start: number; end: number }
  | { kind: 'text'; text: string; position: Point; direction: Point; up: Point; font: Font | null }
...
type Sheet = { id: string; index: number; name: string; status: string; size_in_source_units: [number, number] | null;
  items: Item[]; omissions: unknown[]; diagnostics: string[]; sources: unknown[] };
export type DrawingScene = { status: string; sheet_status: string; units: string; qualified: false; complete: false;
  sheets: Sheet[]; images: { reference: number; resource: string | null; status: string; diagnostic: string | null }[] };
...
export function showDrawing(data: DrawingScene): () => void {
  ...
  document.querySelector('.notice')!.textContent = 'Experimental drawing · Partial display · Units and current state unverified';
  ...
  function select(item: Item) {
    selected = item.id;
    el('selected').textContent = item.geometry.kind === 'text' ? item.geometry.text : 'Drawing element selected';
  ...
  function rows() {
    ...
    if (!query) return;
    const matches = (sheet?.items ?? []).filter(i => i.geometry.kind === 'text' && i.geometry.text.toLocaleLowerCase().includes(query));
    results.append(label('p', `${matches.length} matching text elements`));
    for (const i of matches) { const button = label('button', (i.geometry as Extract<Geometry, {kind: 'text'}>).text); button.onclick = () => select(i); results.append(button); }
  }
  function draw() {
    svg.replaceChildren();
    ...
    for (const item of sheet.items) {
      ...
      if (g.kind === 'image') {
        ...
        const d = g.direction, u = g.up, f = g.font, factor = f?.width_factor ?? 1;
        e = node('text', { transform: `matrix(${d[0] * factor} ${-d[1] * factor} ${-u[0]} ${u[1]} ${g.position[0]} ${H - g.position[1]})`,
          'font-family': f?.family ?? 'sans-serif', 'font-size': f?.height_candidate ?? .25, 'font-weight': f?.weight_candidate ?? 400, fill: color });
  ...
  for (const s of data.sheets) {
    ...
    button.addEventListener('click', () => choose(s)); tree.append(button);
  }
  ...
  if (data.sheets.length) choose(data.sheets[0]);
  else {
  ...
  return () => { stopped = true; drag = null; svg.replaceChildren(); controls.remove(); svg.remove(); };
}
