...
class DependencyIdentity(unittest.TestCase):
    def graph(self):
        ...
        return {
            'packages': [dict(id=i, name=name, version='0.3.7', source=source, manifest_path='/fixture/'+i+'/Cargo.toml')
    ...
    def test_registry_graph_and_explicit_development_override(self):
        graph = self.graph()
        self.assertEqual(check_graph(graph)['acis-core']['version'], '0.3.7')
    ...
    def test_second_core_identity_is_rejected_only_when_linked(self):
        graph = self.graph()
        graph['packages'].append(dict(id='other-core', name='acis-core', version='0.3.7', source=None, manifest_path='/other/Cargo.toml'))
    ...
    def test_previous_dependency_version_is_rejected(self):
        for name in ('acis-core', 'acis-py-bridge'):
            graph = self.graph()
            next(p for p in graph['packages'] if p['name'] == name)['version'] = '0.3.6'
            with self.subTest(name=name), self.assertRaisesRegex(ValueError, '0.3.7'):
                check_graph(graph)
...
class DevelopmentCoreDependencyTests(unittest.TestCase):
    def test_local_core_needs_its_own_explicit_opt_in(self):
        ...
        self.assertEqual(check_graph(graph,allow_local_core=True)['acis-core']['version'],'0.3.7')
